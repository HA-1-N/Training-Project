...
const TableContentUser = ({ handleFilterTableChange, valueFilterList, getUserDataTable, postDeleteUser }: any) => {
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelecteds = totalUserData?.map((n: any) => n.profile_id);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - totalUserData?.length) : 0;

  const handleDeleteCheck = () => {
    postDeleteUser({
      params: selected?.map((item: any) => {
        return {
          id: item,
          delete: 1,
        };
  ...
  const handleChangePaginantion = (event: unknown, newPage: number) => {
    const newData = {
      ...valueFilterList,
      page: newPage,
    };
    ...
    <>
      <Box sx={{ width: '100%', height: 'auto' }}>
        <Paper sx={{ width: '100%', mb: 2, height: 'auto' }}>
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell component="th" id={labelId} scope="row" padding="none">
                          ...
                          <p style={{ margin: '0' }}>{user.storeName}</p>
                        </TableCell>
                        ...
                        <TableCell align="left">
                          <div>{Number(user?.order?.order_as_buyer) + Number(user?.order?.order_as_buyer_total)}</div>
                        </TableCell>
                        ...
                        <TableCell align="right" sx={{ width: '6%' }}>
                          <Grid sx={{ borderLeft: '1.5px dashed #bbb' }}>
                            <Grid>
                              <ButtonDeleteItem onClick={(event) => handleClick(event, user?.profile_id)}>
                                <FaTrash />
                              </ButtonDeleteItem>
                            </Grid>
                          </Grid>
          ...
          >
            <Pagination count={10} color="secondary" onChange={handleChangePaginantion} />
      ...
      {selected.length === 0 ? (
        ...
        <ButtonDelete style={{ opacity: 1 }} onClick={handleDeleteCheck}>
          Remove Selected
        </ButtonDelete>
      )}
    </>
